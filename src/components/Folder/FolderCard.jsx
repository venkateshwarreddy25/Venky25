...
const IconGitHub = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217
      .682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158
      -1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032
      .892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113
      -4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0
      .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337
...
export default function FolderCard({ project }) {
  const { title, description, tags, liveUrl, githubUrl, color } = project;
  ...
  return (
    ...
    >
      ...
      <div className="folder-body">
        <div className="folder-accent-line" aria-hidden="true" />

        <h3 className="folder-title">{title}</h3>
        <p className="folder-desc">{description}</p>

        {/* Tech stack pills */}
        <div className="folder-tags" aria-label="Tech stack">
          {tags.map((tag) => (
